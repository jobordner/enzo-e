...
void Config::read_balance_ (Parameters * p) throw()
{
  ...
  balance_type = p->value ("Balance:type","charm");
...
void Config::read_monitor_ (Parameters * p) throw()
{
  ...
  for (int i=0; i<num_mute; i++) {
    monitor_mute_list.push_back
      (p->list_value_string(i,"Monitor:mute_list","none"));
  }
  int num_only = p->list_length("Monitor:only_list");
  for (int i=0; i<num_only; i++) {
    monitor_only_list.push_back
      (p->list_value_string(i,"Monitor:only_list","none"));
  }
}
...
void Config::read_solver_ (Parameters * p) throw()
{
  ...
  for (int index_solver=0; index_solver<num_solvers; index_solver++) {
    ...
    solver_type[index_solver] = p->value_string
