...
void MethodATS::compute( Block * block) throw()
{
  ...
  if (block->is_leaf()) {
    ...
    test_values_(block,array_curr,error_curr,mx,my,mz,gx,gy,gz);
    ...
    test_history_(array_curr,array_prev,error_curr,mx,my,mz,gx,gy,gz,level,dt);
...
void MethodATS::test_values_(Block * block,
                             ...
                             int gx, int gy, int gz)
{
  ...
  static int count = 0;
  if (err) {
    CkPrintf ("%d METHOD_ATS %s %d values mismatch e.g. %12.10g != %12.10g\n",
              CkMyPe(), block->name().c_str(),count_err,value,time);
    if (count++ >= max_count) {
      ASSERT1("MethodATS","Value mismatch count exceeded %d; exiting!",
              max_count , (err != true));
    }
  }
}
                             ...
                             int gx, int gy, int gz)
{
  ...
  int num_err_total = 0;
  cello_float vxm,vym,vzm;
  cello_float vxp,vyp,vzp;
  int i3[3];
  ...
  for (int axis=0; axis<cello::rank(); axis++) {
    for (int face=0; face<2; face++) {
      if (num_err_face[axis][face]) {
        CkPrintf ("%d METHOD_ATS %s %c%c ghost mismatch "
                  "%g != %g level %d:%d\n",
  ...
  if (num_err_total && (count++ >= max_count)) {
    ASSERT1("MethodATS","Ghost zone mismatch count >= %d",
            max_count , (err != true));
  }
}
