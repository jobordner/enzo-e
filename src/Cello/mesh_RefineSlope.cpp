...
int RefineSlope::apply ( Block * block ) throw ()
{
  ...
  for (size_t k=0; k<field_id_list_.size(); k++) {
    ...
    cello_float * array = field.values(id_field);
    // count number of times slope refine and coarsen conditions are satisified

    cello_float slope;
    const int d3[3] = {1,mx,mx*my};
    cello_float tiny = 1e-10;
    int count=0;
    // cello_float min = std::numeric_limits<cello_float>::max();
    // cello_float max = 0.0;
      ...
      for (int iz=gz; iz<mz-gz; iz++) {
        for (int iy=gy; iy<my-gy; iy++) {
          for (int ix=gx; ix<mx-gx; ix++) {
            ...
            cello_float a = std::max(cello_float(2.0*h3[axis]*fabs(array[i])),tiny);
            ...
            if (output) {
              if (slope > max_coarsen_) output[i] =  0.0;
              if (slope > min_refine_)  output[i] =  1.0;
            }
          }
        }
      }
