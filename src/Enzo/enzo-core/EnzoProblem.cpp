 ...
 ) throw ()
{
  ...
  if (type == "hdf5") {
    ...
    parameter_path_type current_group;
    current_group.push_back("Initial");
    current_group.push_back(config->initial_list[index]);

    initial = new EnzoInitialPm
      (parameters, current_group,"mask",
       cycle,time,
...
/// @param config Configuration parameters class
{
  CkPrintf ("DEBUG_PARAM EnzoProblem::create_method %d %s\n",index_method,name.c_str());
  Method * method = 0;
  ...
  current_group.push_back(mlist[index_method]);
  ParameterGroup p_group(*parameters, current_group);
  ...
  if (name == "ppm") {
    ...
    CkPrintf ("DEBUG_PARAM p_group = %s\n",p_group.full_name("solver").c_str());
