...
void EnzoInitialBCenter::initialize_values_()
{
  ...
  for (int i = 0; i < 3; i++){
    if (parameters_->type(path,names[i]) == parameter_unknown){
      values_[i] = nullptr;
    } else {
      if ( (parameters_->type(path,names[i]) == parameter_list) &&
	   (parameters_->list_length(path,names[i]) == 0)){
	values_[i] = nullptr;
      }
      values_[i] = new Value(parameters_, path,names[i]);
    }
  }
}
