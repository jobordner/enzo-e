...
static float group_times[1000] = {0};
static float restart_times[20]={0};
...
void IoEnzoReader::p_create_level (int level)
{
  ...
  int i=0;
#define PERF_TIME_START(i) restart_times[i] = cello::simulation()->timer() - restart_times[i];
#define PERF_TIME_STOP(i) restart_times[i] = cello::simulation()->timer() - restart_times[i];
  ...
  for (int k=0; k<20; k++) {
    if (restart_times[k] != 0) { CkPrintf ("%g PERF_TIME proc %d step %d\n",restart_times[k],CkMyPe(),k); }
  }
  for (int k=0; k<1000; k++) {
    if (group_times[k] != 0) { CkPrintf ("%g GROUP_TIME proc %d step %d\n",group_times[k],CkMyPe(),k); }
  }
  // self
  block_created_();
}
...
#define GROUP_TIME_START(i) group_times[i] = cello::simulation()->timer() - group_times[i];
#define GROUP_TIME_STOP(i) group_times[i] = cello::simulation()->timer() - group_times[i];
    GROUP_TIME_START(i);
    void * buffer;
    std::string name;
    int type_scalar;
